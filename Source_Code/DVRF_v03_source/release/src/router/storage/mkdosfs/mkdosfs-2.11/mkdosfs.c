...
static unsigned long blocks;		/* Number of blocks in filesystem */
static int sector_size = 512;	/* Size of a logical sector */
static int sector_size_set = 0; /* User selected sector size */
static int backup_boot = 0;	/* Sector# of backup boot sector */
static int reserved_sectors = 0;/* Number of reserved sectors */
static unsigned long badblocks = 0;	/* Number of bad blocks in the filesystem */
...
static unsigned long currently_testing = 0;	/* Block currently being tested (if autodetect bad blocks) */
static struct msdos_boot_sector bs;	/* Boot sector data */
static unsigned long start_data_sector;	/* Sector number for the start of the data area */
static unsigned long start_data_block;	/* Block number for the start of the data area */
static unsigned char *fat;	/* File allocation table */
static unsigned char *zero_fat;
...
static long do_check (char *buffer, int try, long current_block);
static void alarm_intr (int alnum);
static void check_blocks (void);
static void get_list_blocks (char *filename);
static int valid_offset (int fd, loff_t offset);
static unsigned long count_blocks (char *filename);
...
static long
do_check (char *buffer, int try, long current_block)
{
  long got;
...
establish_params (int device_num,int size)
{
  ...
  if ((0 == device_num) || ((device_num & 0xff00) == 0x0200))
	...
	//unsigned int volsize = blocks*SECTORS_PER_BLOCK;
	loff_t  volsize = blocks*SECTORS_PER_BLOCK;
	if (volsize <= 66600)
	...
	bs.cluster_size = 4;
    	...
    	printf("%s: cluster %d, secs_track %d, heads %d, sector %d:%d\n ",	__FUNCTION__,
		bs.cluster_size, bs.secs_track,bs.heads, bs.sectors[0], bs.sectors[1]);
}
...
#else
	fprintf(stderr,"setup_tables_fat_length==[%0x]\n",fat_length);
...
write_tables (void)
{
  ...
  seekto( reserved_sectors*sector_size, "first FAT" );
	...
	fprintf(stderr,"write_tables_fat_length==[%0x],nr_fats=[%0x]\n",fat_length,nr_fats);
	
	for (x = 1; x <= nr_fats; x++) 
	{
		static unsigned char *zero_fat_local = NULL;
		unsigned temp = 0;
		...
		#else
		fprintf(stderr,"write_buf_512K\n");

		if(fat_length > 1024)
		{
			if ((zero_fat_local = (unsigned char *) malloc (sector_size*1024)) == NULL)
			{
				temp = 0;
			}
			else
			{
				memset( zero_fat_local, 0, sector_size*1024 );
				temp = 1024;
			}
		...
		for (fatno = 1; fatno < fat_length; fatno++)
		{
			if(((fat_length - fatno)> 1024 )&& (temp !=0))
			{
				fatno = fatno + temp -1 ;
				writebuf( zero_fat_local, sector_size*1024, "FAT" );
			}
			else
			{
				writebuf( zero_fat, sector_size, "FAT" );
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt (argc, argv, "AcCf:F:Ii:l:m:n:r:R:s:S:v")) != EOF)
	...
	printf("mkdosfs: i=%lu, optind=%d, argc=%d\n", i, optind, argc);
  	if (optind == argc - 2)	/*  Either check the user specified number */
    	{
      		blocks = (unsigned int) strtol (argv[optind + 1], &tmp, 0);
      		if (!create && blocks != i)
		{
