...
{
    fprintf(stderr,"usage: %s [-aAflrstvVwy] [-d path -d ...] "
      "[-u path -u ...]\n%15sdevice\n",name,"");
    ...
    fprintf(stderr,"  -s       show the volume label, if any, and exit\n");
...
extern int filter_printf(const char *format, ...)
{
    ...
    const char *follow_old;
    char *follow_new;
    va_list ap;
    int result;

    fprintf(stdout, "<script>check_fat_filesystem_info(\"");
    follow_old = format;
    follow_new = new_format;
    while (*follow_old != 0)
    {
	if (*follow_old == '\n')
	{
...
{
    DOS_FS fs;
    int rw,salvage_files,verify,show_label,c;
    unsigned long free_clusters;
    int print_volume_name = 0;
    char volume_name[12];
    int entry_bit = 0;

    rw = salvage_files = verify = show_label = 0;
    interactive = 0;
    check_atari();

    while ((c = getopt(argc,argv,"Aabd:flmprstu:vVwy")) != EOF)
	switch (c) {
		...
		salvage_files = 1;
		break;
	    /* Show FAT's number of entry bit. 
	     * John.Huang 2011.07.08
	     */
	    case 'b':
		entry_bit = 1;
		break;
    ...
    read_boot(&fs);
    if (entry_bit == 1)
	fprintf(stdout, "Entry bit: %02d\n", fs.fat_bits);
    if (show_label) {
	if (!scan_root_for_volume_label(&fs))
	    fprintf(stdout, "No volume label found.\n");
	fprintf(stdout, "Volume ID: 0x%02x%02x%02x%02x\n",
		(unsigned)(fs.volume_id[0]), (unsigned)(fs.volume_id[1]),
		(unsigned)(fs.volume_id[2]), (unsigned)(fs.volume_id[3]));
	qfree(&mem_queue);
