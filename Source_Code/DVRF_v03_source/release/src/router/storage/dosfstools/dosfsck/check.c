...
static int check_dir(DOS_FS *fs,DOS_FILE **root,int dots)
{
	...
	skip = !!((*walk)->dir_ent.attr & ATTR_VOLUME);
	scan = &(*walk)->next;
	while (*scan && !skip) {
...
static int check_dir_for_volume_label(DOS_FS *fs,DOS_FILE **root,int dots)
{
    DOS_FILE **walk;
    char volume_name_buffer[12];
    int position;

    walk = root;
    while (*walk) {
	if ((*walk)->dir_ent.attr == ATTR_VOLUME) {
		memcpy(volume_name_buffer, (*walk)->dir_ent.name, 8);
		memcpy(&(volume_name_buffer[8]), (*walk)->dir_ent.ext, 3);
		position = 11;

		while ((position > 0) && (volume_name_buffer[position - 1] == ' '))
			--position;

		volume_name_buffer[position] = 0;
		/* 2011-08-15, patch E4200 IR-B0017572, Storage feature doesn't work when disk partition names is all space. */
		if (position ==1 && (unsigned char)volume_name_buffer[0] == 0xe5)
...
static void add_file_for_volume_label(DOS_FS *fs,DOS_FILE ***chain,
					 DOS_FILE *parent,
					 loff_t offset,FDSC **cp)
{
    DOS_FILE *new;
...
static int subdirs_for_volume_label(DOS_FS *fs,DOS_FILE *parent,FDSC **cp);
...
static int scan_dir_for_volume_label(DOS_FS *fs,DOS_FILE *this,FDSC **cp)
{
    DOS_FILE **chain;
    int i;
    unsigned long clu_num;
    ...
    new_dir();
    while (clu_num > 0 && clu_num != -1) {
	add_file_for_volume_label(fs,&chain,this,cluster_start(fs,clu_num)+(i % fs->
	  cluster_size),cp);
	i += sizeof(DIR_ENT);
	if (!(i % fs->cluster_size))
	    if ((clu_num = next_cluster(fs,clu_num)) == 0 || clu_num == -1)
    ...
    return check_dir_for_volume_label(fs,&this->first,this->offset);
}
...
static int subdirs_for_volume_label(DOS_FS *fs,DOS_FILE *parent,FDSC **cp)
{
	...
	if (walk->dir_ent.attr & ATTR_DIR)
	    if (strncmp(walk->dir_ent.name,MSDOS_DOT,MSDOS_NAME) &&
	      strncmp(walk->dir_ent.name,MSDOS_DOTDOT,MSDOS_NAME))
		if (scan_dir_for_volume_label(fs,walk,file_cd(cp,
			walk->dir_ent.name))) return 1;
    return 0;
}
...
int scan_root_for_volume_label(DOS_FS *fs)
{
    DOS_FILE **chain;
    int i;
    ...
    new_dir();
    if (fs->root_cluster) {
	add_file_for_volume_label(fs,&chain,NULL,0,&fp_root);
    }
    else {
	for (i = 0; i < fs->root_entries; i++)
	    add_file_for_volume_label(fs,&chain,NULL,fs->root_start+i*sizeof(DIR_ENT),&fp_root);
    }
    if (check_dir_for_volume_label(fs,&root,0))
	return 1;
    return subdirs_for_volume_label(fs,NULL,&fp_root);
}
