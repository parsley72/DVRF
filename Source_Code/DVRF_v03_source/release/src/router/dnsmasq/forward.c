...
//add by michael to support the custom 404 page at 20081013
char lan_ipaddr[20];
#ifdef CUSTOM404_SUPPORT
int ctm404_enable;
int wan_link;
char domain_name[MAXDNAME+1];
//add by michael to avoid enabling CWA with non_A type query
static unsigned int check_query_isok(HEADER *header)
{
	int qtype,qclass;
	...
	for(q = 0; q<qdcount; q++)
	{
		//skip the name field,for name zone end with "\0"
		char *cp;
		unsigned j,I,name_match = 1;
		...
		while ((I = *ansp++))
		{
			if (( I & 0xc0) == 0xc0) /* pointer for name compression */
			{
				ansp++;
				name_match = 0;	//A type query name should not be a pointer
				break;
			}
			else           
			{
				if(cp - name + I +1 >= MAXDNAME)
						return 0;
		...
		if(cp == name)
			...
			*--cp = 0;	//add the end mark for the string.

		//check whether it a LAN-side domain query
		if(strlen(domain_name) == 0 || strlen(name) < strlen(domain_name))
		{
			name_match = 0;
		}
		else
		{
			...
			while(len >0)
			{
				cp--;
				len--;
				if(strcasecmp(&domain_name[len],cp) != 0)
				{
					name_match = 0;
					break;
				}
		...
		//check special domain name
		if( strstr(name,".linksys.com") || strstr(name,".linksysbycisco.com") || strstr(name,".cisco.com") )
			name_match = 1;
...
/*
 * New code from Joe
 * Types : T_A, T_NS, T_CNAME, T_SOA, T_PTR, T_MX
 */
static unsigned char *add_ip_record(unsigned int nameoffset, unsigned char *p, unsigned long ttl)
{
	struct in_addr addr;
#if 0
	FILE *fp;
	char tmp[60] = "" ;
	char ip_tmp[40]= "" ;
	/* get the dynamic router address */
        if(fp=fopen("/tmp/dns_url","r"))
        {
                fgets(tmp,sizeof(tmp),fp);
                strcpy(ip_tmp,strtok(tmp,"#"));
                inet_aton(ip_tmp,&addr);
                strcpy(ip_tmp,strtok(NULL,"#")); 
                fclose(fp);
        }

#else
	inet_aton(lan_ipaddr,&addr);
#endif
    PUTSHORT(nameoffset | 0xc000, p);
    PUTSHORT(T_A, p); // TYPE
    PUTSHORT(C_IN, p); // CLASS
    PUTLONG(ttl, p); // TTL
    PUTSHORT(0x04, p); // RDLENGTH
    memcpy(p,&addr,INADDRSZ);
    p+=INADDRSZ;
    PUTSHORT(0x00, p); // NULL
    *p++ = 0;

    return p;
}

static unsigned char *skip_question(HEADER *header, unsigned int plen)
{
    int q, qdcount = ntohs(header->qdcount);
    unsigned char *ansp = (unsigned char *)(header+1);
...
/*
 * Build a DNS return to send to local server
 */
static void send_to_local(int udpfd, HEADER *header, int plen, union mysockaddr *udpaddr)
{
    int m;
    ...
    ftsp = ansp = skip_question(header, plen); // Get to the end of the current stream
    nameoffset = ((unsigned char *)(header+1)) - (unsigned char *)header; // Calculate the start of the question Q_NAME

    ansp = add_ip_record(nameoffset, ansp, 0);

    m = ansp - (unsigned char *)header; // Calculate stream length
    sendto(udpfd, (char *)header, m, 0, &udpaddr->sa, sa_len(udpaddr)); // Respond back
}
#endif
//end by michael
...
#ifdef CUSTOM404_SUPPORT  //add by michael to support the custom 404 page at 20081013
	int cwa_in = check_query_isok(header);
	if(ctm404_enable == 1 && wan_link == 1 && cwa_in == 1)
		  send_to_local(udpfd, header, plen, udpaddr); 
	else
	{
#endif
//remove sending the empty response to the client to fix the Home Page can't work issue at 20100208
/* 2011-08-19 -- Fix [E1550-147] P2, IPv6, "Save the Security Log" doesn't work.
 * IE8 with SmartScreen filter on, it will query for urs. microsoft.com. 
 * When saving Security Log without DNS response, save file window will get stuck for a while.
 * Sync from E4200 to turn on this.
 */
#if 1
...
struct server *reply_query(int fd, char *packet, char *dnamebuff, struct server *last_server,unsigned long *timetolive)
{
  ...
  struct frec *forward;
  HEADER *header;
  int n = recv(fd, packet, DPACKETSZ, 0);
  ...
  if (n >= (int)sizeof(HEADER) && header->qr)
      ...
      if ((forward = lookup_frec(ntohs(header->id))))
	{
	  ...
	  forward->new_id = 0; /* cancel */
		...
		int cwa_in = check_query_isok(header);
		if(ctm404_enable == 1 && wan_link==1 && cwa_in == 1)
			send_to_local(forward->fd, header, n, &forward->source);
		else
		{
#endif
                        /* There's no point returning an upstream reply marked as truncated,
