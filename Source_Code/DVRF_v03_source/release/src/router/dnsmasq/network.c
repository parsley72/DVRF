...
//add by michael to fix the dnsmasq bind error issue(CRDCIR-B0011548) at 20090624
//remove the lo interface to improve the samba access speed at 20100415
	if(!strcmp(ifr->ifr_name,"vlan1:0") || !strcmp(ifr->ifr_name,"eth1:0")|| !strcmp(ifr->ifr_name,"eth2:0") || !strcmp(ifr->ifr_name,"lo"))	
		continue;
     ...
     it shouldn't break on other Unices, and their SIOGIFCONF might work. */
  {
	...
	char devname[20], addrstring[32];
	  ...
	  /* check if all IPv6 address can be used... */
	  while (wait) {
		  wait = 0;
		  ...
		  if ((f = fopen(IP6INTERFACES, "r")))
		  {
					  ...
					  addrstring, &if_idx, &plen, &scope, &flags, devname) != EOF) 
			  {
				  ...
				  { 
					  struct iname *tmp;
					  for(tmp = names; tmp; tmp = tmp->next)
						  // IFA_F_TENTATIVE is 0x40, in if_addr.h
						  if (flags & 0x40 && strcmp(tmp->name, devname) == 0)
						  {
							  wait = 1;
							  fprintf(stderr, "[Dnsmasq], %s is IFA_F_TENTATIVE, wait for 1 sec.\n", addrstring);
							  sleep(1);
							  break;
						  }
				  }
	...
	while (fscanf(f, "%32s %02x %02x %02x %02x %20s\n",
		      addrstring, &if_idx, &plen, &scope, &flags, devname) != EOF) 
	  {
	    ...
	    addr.in6.sin6_port = htons(port);
	    addr.in6.sin6_flowinfo = htonl(0);
	    addr.in6.sin6_scope_id = /*htonl(scope)*/ if_idx;  //2010-12-14 ruby modified
...
#endif /* defined(HAVE_LINUX_IPV6_PROC) && defined(HAVE_IPV6) */
