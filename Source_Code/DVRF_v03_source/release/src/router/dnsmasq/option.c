...
#endif
"-R, --read_peerfd           Use random wan port sent to DNS server.\n"
"\n";
  ...
  while (1)
...
#endif
#ifdef HAVE_GETOPT_LONG
	option = getopt_long(argc, argv, "owefnbvhdqRr:m:p:c:l:s:i:t:u:a:x:S:C:T:", 
			     (struct option *)opts, NULL);
#else
        option = getopt(argc, argv, "owefnbvhdqRr:m:p:c:l:s:i:t:u:a:x:S:C:T:");
#endif
#ifdef HAVE_FILE_SYSTEM
      else
      	...
      	option == 'w')
	{
      ...
      if (
        ...
        {
          *read_peerfd = 0;
          fprintf (stderr, "dnsmasq use dynamic port\n");
          break;
        }     
#endif      
