...
//			printf("prefered ia_na = %d %x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x\n", prefered_ia_na.address.len, 
...
//			printf("prefered ia_pd = %d %x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x\n", prefered_ia_pd.address.len, 
...
do_refresh6(void *input)
{
	...
	if ((client->refresh_type == DHCPV6_RENEW) && client->MRD && (tv.tv_sec > client->start_time.tv_sec + client->MRD)) {
