...
static void send_renew_event(void)
{
	dhcpclient_action(SEND_REQUEST_PACKET);	
}

static void send_release_event(void)
{
...
static void signal_handler(int sig)
{
	...
	switch (sig) 
	{
		case SIGUSR1:	//renew
			//dhcpclient_action(SEND_REQUEST_PACKET);
			add_timeout(&tv, send_renew_event, 0, 0, 0);
			break;
		case SIGUSR2:	//release
			//dhcpclient_action(SEND_RELEASE_PACKET);
			add_timeout(&tv, send_release_event, 0, 0, 0);
			break;
		case SIGTERM:
		default:
	...
	struct client_state *client;
{
	...
	//add by michael to support bridge_mode at 20110711
	if(bridge_mode == 1 && client -> state == S_REQUESTING)
	{
		begin_flag = 0;
		begin_time = 0;
		setnoip_flag = 0;
