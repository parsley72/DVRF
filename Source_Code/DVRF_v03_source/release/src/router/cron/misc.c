...
static char rcsid[] = "$Id: misc.c,v 1.4 2007/07/05 09:35:53 michael Exp $";
#endif
	...
	char	*detail;
{
	...
	if (LogFD < OK) {
		LogFD = open(CRON_LOG_FILE, O_WRONLY|O_APPEND|O_CREAT, 0600);
		if (LogFD < OK) {
			fprintf(stderr, "%s: can't open log file\n",
				ProgramName);
			perror(CRON_LOG_FILE);
		} else {
	...
	time_t *clock;
{
	...
	time_t t = clock ?*clock :time_wrap(0L);
