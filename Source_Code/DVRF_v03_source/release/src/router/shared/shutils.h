 ...
 *
 * $Id: shutils.h,v 1.14 2007/06/01 05:59:05 michael Exp $
 */
...
/* Copy each token in wordlist delimited by token into word */
#define foreachtoken(word, wordlist, next, token) \
	for (next = &wordlist[strspn(wordlist, token)], \
	     strncpy(word, next, sizeof(word)), \
	     word[strcspn(word, token)] = '\0', \
	     word[sizeof(word) - 1] = '\0', \
	     next = strstr(next, token); \
	     strlen(word); \
	     next = next ? &next[strspn(next, token)] : "", \
	     strncpy(word, next, sizeof(word)), \
	     word[strcspn(word, token)] = '\0', \
