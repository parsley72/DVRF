...
//John@2010.03.13
static const char default_home_prefix[] = "/tmp/ftproot/";
int pf = 0; 
char passwords[128];
...
static void passwd_wrapper(const char *login)
{
   ...
   //John@2010.03.15
    if(pf){
       BB_EXECLP(prog, prog, "-p",login, passwords,NULL);
    }else
...
{
 enum {
        OPT_passwd = 0x01, //John@2010.03.13,password option
    };
    unsigned opt;
	const char *login = NULL; //John
	struct passwd pw;
	...
	opt_complementary = "=1";
	/*John@2010.03.*/
	opt = getopt32(argv, "p:h:g:s:G:DSH",&login, &pw.pw_dir, &pw.pw_gecos, &pw.pw_shell, &usegroup);
	argv += optind;
  ...
  /* get passwords */
    if (opt & OPT_passwd) {
        ...
        if ((optind+1) < argc) {
            ...
            strncpy(passwords, argv[0], sizeof(passwords));
        }
	}

	if (!pw.pw_dir) {
		/* create string for $HOME if not specified already */
		pw.pw_dir = xasprintf("%s%s", default_home_prefix,login/*argv[0]*/);//John
	}
	pw.pw_passwd = (char *)"x";
	pw.pw_uid = 0;
