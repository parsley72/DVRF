...
#include <syslog.h>
/*John@2010.03.15*/
static char enpasswords[128];

static void nuke_str(char *str)
{
	if (str) memset(str, 0, strlen(str));
}
/*John@2010.03.15 add -p option support*/
static char* new_password(const struct passwd *pw, uid_t myuid, int algo,int flag)
{
...
int passwd_main(int argc, char **argv)
{
    enum {
        ...
        OPT_lud = 0xe,
        OPT_pw = 0x10, //John@2010.03.13,password option
        //STATE_ALGO_des = 0x20, not needed yet
        STATE_ALGO_md5 = 0x40,
    };
    enum { sizeof_passwd = 128 };  //John
	...
	opt = getopt32(argv, "a:ludp", &opt_a); //John@2010.03.15,add -p option support
	//argc -= optind;
	argv += optind;
    ...
    //John@2010.03.15
   if ((opt&OPT_pw) && ((optind+1) >= argc)) {
            bb_error_msg_and_die( "no password specified");
   }
	...
	if (!(opt & OPT_lud)) {
        ...
        //John@2010.03.15 support -p option for passwd
        if(opt&OPT_pw){
            strcpy(enpasswords,argv[optind-1]);
        }
		printf("Changing password for %s\n", name);
		newp = new_password(pw, myuid, opt & STATE_ALGO_md5,opt & OPT_pw);
		if (!newp) {
