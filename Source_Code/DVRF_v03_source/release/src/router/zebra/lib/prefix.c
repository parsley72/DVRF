...
/*
  prefix_checkip()  -- Reject the IP addresses that are reserved for
  other things  Fixed 03/11/2007
*/
int prefix_checkip(const struct in_addr ip)
{
   ...
   {                                              // 240-254.x.x.x == reserved, experimental
       zlog_info("Is valid ip Address, false, %s\n", inet_ntoa(ip));
       return 0;
   }
   zlog_info("Is valid ip Address, true, %s\n", inet_ntoa(ip));
   return 1;
}

/*
  prefix_contiguousSubnet() - verify that a subnet mask no holes in it.
   Fixed 03/11/2007
*/
int prefix_contiguousSubnet(const struct in_addr mask)
{
    // Convert aaa.bbb.ccc.ddd into ddd.ccc.bbb.aaa
    int 	 bContinuous;
    u_int32_t	 i, dwContiguousMask;
    u_int32_t 	 hmask = ntohl(mask.s_addr);
...
///////////////////////////////////////////////////////////////////////////////
// prefix_validIPConfig()  -- Determine if the given IPAddress, mask, and gateway
//                       make sense.  Fixed 03/11/2007
///////////////////////////////////////////////////////////////////////////////
int prefix_validIPConfig(const struct in_addr address,
		     const struct in_addr mask)
{
	...
	// reserved addresses.
	if (!prefix_checkip(address)) {
	    zlog_info("Is valid IP config, address, or gateway was invalid (reserved) IP\n");
	    break;
	}
	
	if (!prefix_contiguousSubnet(mask)) {
	    zlog_info("Is valid IP Config, mask was not contiguous\n");
	    break;
	}

	//Fixed 07/05/2007 remove check host ip address can't setting 
	//if ( (~mask.s_addr) + (address.s_addr & mask.s_addr) == address.s_addr) {
	//if (!(address.s_addr & mask.s_addr))
	//Fixed 07/12/02 Subnet mask with value of zero to class A,B and C
	if ((!(address.s_addr & mask.s_addr)) && (mask.s_addr != 0))
	{
	    zlog_info("You can't have an IP address at 255\n");
	    break;
	}
	bIsValid = 1;
    } while (0);
