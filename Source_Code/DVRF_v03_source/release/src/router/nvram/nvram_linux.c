...
int
nvram_ck_set(const char *name, const char *value)
{
	if(nvram_match("manual_nv", "1"))
		return 0;
...
nvram_set(const char *name, const char *value)
{
	 extern struct nvram_convert nvram_converts[];
         ...
         for(v = nvram_converts ; v->name ; v++) {
			 ...
			 //Jemmy add for dual band wireless macfilter 2008.5.15
			if(strcmp(v->wl1_name,""))	_nvram_set(v->wl1_name, value);
                        if(strcmp(v->d11g_name,""))     _nvram_set(v->d11g_name, value);
                 }
		else if(!strcmp(v->wl0_name, name) || !strcmp(v->wl1_name, name)){
			if(strcmp(v->name,""))      _nvram_set(v->name, value);
		}
         }
