...
"$Id: debug.c,v 1.3 2007/08/07 06:47:11 jack Exp $";
...
{
    static char buf[64];
    sprintf(buf, "my_id[%d]/assigned_id[%d]", (int) tunnel->my_id, (int) tunnel->assigned_id);
    return buf;
}
...
{
    static char buf[128];
    sprintf(buf, "(session->tunnle myid[%d]/assign_id[%d],session myid[%d]/assigned_id[%d])",
