...
"$Id: tunnel.c,v 1.5 2007/08/20 09:59:49 jack Exp $";
...
tunnel_set_state(l2tp_tunnel *tunnel, int state)
{
    ...
    DBG(l2tp_db(DBG_TUNNEL, "%s() tunnel(%s) state %s -> %s\n",
	   __FUNCTION__,l2tp_debug_tunnel_to_str(tunnel),
...
tunnel_establish(l2tp_peer *peer, EventSelector *es)
{
    ...
    DBG(l2tp_db(DBG_TUNNEL, "@.@Link_Entry tunnel_establish() -> %s\n", l2tp_debug_tunnel_to_str(tunnel)));
					     ...
					     struct sockaddr_in *from)
{
	...
	t.tv_sec = 60;
	t.tv_usec = 0;
        DBG(l2tp_db(DBG_TUNNEL, "@.@%s()- chang_time ?? tunnel -> %s\n", __FUNCTION__, l2tp_debug_tunnel_to_str(tunnel)));
		      ...
		      void *data)
{
	...
	tunnel_free(tunnel);
	//system("ppp_event -t L2TP_SCCRQ_TIMEOUT &");
	return;
    }
		  ...
		  l2tp_dgram *dgram)
{
    ...
    DBG(l2tp_db(DBG_TUNNEL, "%s() %s: Peer host name is '%s'\n",
                           __FUNCTION__, l2tp_debug_tunnel_to_str(tunnel), tunnel->peer_hostname));
