...
"$Id: sync-pppd.c,v 1.2 2005/08/08 07:09:29 kanki Exp $";
...
close_session(l2tp_session *ses, char const *reason, int may_reestablish)
{
    ...
    //Jemmy add for fix issues: restart l2tp login several times when user name or password is error 2008.5.8	
    memset(buf, 0, sizeof(buf));
    if(file_to_buf("/tmp/ppp/log", buf, sizeof(buf)))
    {
	if (!strstr(buf, "L2TP"))
	{
		if (l2tp_redial_times < L2TP_REDIAL_MAXTIMES)
		{			
