/*
 *   $Id: send.c,v 1.1 2011/03/21 10:55:28 eric.ma Exp $
 *
 *   Authors:
...
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	{// 2011-0516 add to "Test CPEInterop 3.4: No Default Route Part A & Part B"
	    FILE *fp=NULL;
	    unsigned char iproute_cmd[]="/usr/sbin/ip -6 route show default";
	    unsigned char output[256]={0};

	    if ((fp = popen(iproute_cmd, "r")) != NULL) {
		if (fgets(output, sizeof(output), fp) == NULL) {
			/* if no default route, send zero router lifetime */
