...
on subnet %s being rejected as it is one of our names.\n", 
	...
	#if NMBD_NAME_REGISTER_SUPPORT
     		if(!group && (question->name_type == 0x0 || question->name_type == 0x20)){ //Which types will be supported? please fix me
			delete_netbios_name_from_list(question->name, inet_ntoa(owner_ip));
     		}
	#endif
		return;
	}
...
IP %s on subnet %s\n", nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name));
	//cprintf("process_name_registration_request: Name registration for name %s \
IP %s on subnet %s\n", nmb_namestr(question), inet_ntoa(from_ip), subrec->subnet_name);
...
with IP %s on subnet %s\n",nmb_namestr(&namerec->name),inet_ntoa(from_ip), subrec->subnet_name));
		...
		#if NMBD_NAME_REGISTER_SUPPORT
    			if(question->name_type == 0x0 || question->name_type == 0x20)  //Which types will be supportted? please fix me
			{
				cprintf("before write %s, from_ip = %s\n", question->name, inet_ntoa(from_ip));
    				write_netbios_name_to_list(question->name, inet_ntoa(from_ip));
		}
		#endif
		return;
