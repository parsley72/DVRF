	...
	*ppdata = (char *)SMB_REALLOC(
	...
	switch (info_level) {
		case SMB_INFO_ALLOCATION:
		{
			...
			if (strequal(lp_servicename(snum),"E4200USB")
				&& !StrnCaseCmp(lp_pathname(snum),"/foreign_shares/USB", 
				strlen("/foreign_shares/USB"))
				&& (strlen(lp_pathname(snum)) == strlen("/foreign_shares/USBXXXXXX"))) {
				
				fstrcpy(dirpath, conn->dirpath);
				if(strequal(conn->dirpath, "") || strequal(conn->dirpath, "./"))
					fstr_sprintf(path,"%s/%s", lp_pathname(snum), lp_servicename(0));
				else
					fstr_sprintf(path,"%s/%s", lp_pathname(snum),strsep(&dirp, "/"));
			}else if (strequal(lp_servicename(snum),"E3200USB")
				&& !StrnCaseCmp(lp_pathname(snum),"/foreign_shares/USB", 
				strlen("/foreign_shares/USB"))
				&& (strlen(lp_pathname(snum)) == strlen("/foreign_shares/USBXXXXXX"))) {
