    ...
    // data is available on a socket.
{
   ...
   // GetPeerMac(skt,buf);
    ...
    if (packetLen >= 0) {
		...
		strcpy(InterfaceName,intf->intfName);//record Interface
...
static int SetupInterfaceList(mDNS *const m)
{
    ...
    if (err == 0) {
        ...
        while (thisIntf != NULL) {
		    ...
		    if(strncmp(thisIntf->ifi_name,"br",2) == 0) //only for br+ build socket.
...
mDNSexport void mDNSPosixGetFDSet(const mDNS *const m, int *nfds, fd_set *readfds, struct timeval *timeout)
	...
	#if 0 // wangfei remove here for infulencing performance
