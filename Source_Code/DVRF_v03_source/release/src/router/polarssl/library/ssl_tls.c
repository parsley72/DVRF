...
int ssl_init( ssl_context *ssl )
{
	...
	/* Lai 2010.04.16 add to init the cache. */
    memset( ssl->ssl_buffer, 0, SSL_MAX_BUF_SIZE);
	ssl->ssl_buflen = 0;
...
/*
 * Read one line from the buffer pool to parse the http head
 *
 * Lai 2010.04.16
 *
 */
int ssl_read_line( ssl_context *ssl, unsigned char *buf, int len )
{
	...
	cur_para = malloc(sizeof(char) * (len + 10) );
	if(cur_para == NULL)
		return -1;
    memset(cur_para, 0, len + 10);

    memcpy( cur_para, ssl->in_offt, n );
	cur_ptr = strstr(cur_para, "\r\n");
	if(cur_ptr)
	{
		bytes = cur_ptr - cur_para;
 ...
 */
int _ssl_write( ssl_context *ssl, const unsigned char *buf, int len )
{
    int ret, n;
...
/*
 * Flush any data on user buffer cache
 *
 * Lai 2010.04.16 add
 */
int ssl_flush( ssl_context *ssl )
{
...
/*
 * Copy ssl_write to _ssl_write
 * Now in this function, will temporary store the message to the 
 * cache, or send the message out when it exceed the max cache size.
 *
 * Why do that?
 * Some actions in httpd, it will make this call continually to send
 * short sentences, cache it to the larger buffer and send it out in
 * one time will great improve the performance.
 *
 * Lai 2010.04.16 add
 *
 */
int ssl_write( ssl_context *ssl, const unsigned char *buf, int len )
{
	...
	SSL_DEBUG_MSG( 2, ( "=> first write %d", len ) );
	if(ssl == NULL || buf == NULL || len == 0)
		return 0;

	if(ssl->ssl_buflen + len <= SSL_SEND_BUF_SZ)
	{
		/* Enough buffer to store the message, just copy it to our cache. */
	...
	{
		/* No enough cache, clear cache first. */
		if(ssl->ssl_buflen)
			ssl_flush(ssl);
