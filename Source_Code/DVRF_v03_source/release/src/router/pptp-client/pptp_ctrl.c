 ...
 *
 * $Id: pptp_ctrl.c,v 1.5 2007/12/12 06:33:42 jack Exp $
 */
 ...
 */
u_int8_t PPTP_CALL_CLEAR_RQST_FLAG=0;  //2006.12.19 add for cdrouter v3.3 item 28(cdrouter_pptp_30) bug
...
//static int stop_flag = 0; //@.@20071212 [0 -> send call_clear_request  1-> stop_control_connection_request]
static short peer_id = 0; /*@.@ keep handshake peer_id for set_link_info packet (DUT --> Server)*/
struct PPTP_CONN {
  int inet_sock;
...
PPTP_CONN * pptp_conn_open(int inet_sock, int isclient, pptp_conn_cb callback)
{
  ...
  if (isclient) {
    ...
    /*@.@Start connect to PPTP-Server PhaseII*/
    int idx, rc;
  ...
  sigact.sa_handler = pptp_handle_timer; /*@.@ handled echo-request & recv echo-reply Phase III*/
			   ...
			   pptp_call_cb callback, char *phonenr)
{
  ...
  /* Assign call id */
  if (!vector_scan(conn->call, 1 , PPTP_MAX_CHANNELS-1, &i)) /*@.@Original default call_id =0 */
...
void pptp_call_close(PPTP_CONN * conn, PPTP_CALL * call) {
 /**** 20081231 modify by zhaoguang for the bug (ID:17470) 
  **** that DUT doesn't send Stop-Control-Connection-Request message and TCP FIN 
  **** to PPTP server when you click the "Disconnect" button in the Status window. ****/
  //struct pptp_call_clear_rqst rqst = { PPTP_HEADER_CTRL(PPTP_CALL_CLEAR_RQST), 0, 0 };
  struct pptp_stop_ctrl_conn stop_rqst = { PPTP_HEADER_CTRL(PPTP_STOP_CTRL_CONN_RQST), htons(1), 0, 0};
  int ret = 0; 
   ...
   * requests to be made.
   */
  PPTP_CALL_CLEAR_RQST_FLAG=1;  //2006.12.19 add for cdrouter v3.3 item 28(cdrouter_pptp_30) bug
  /**** 20081231 modify by zhaoguang for the bug (ID:17470) 
   **** that DUT doesn't send Stop-Control-Connection-Request message and TCP FIN 
   **** to PPTP server when you click the "Disconnect" button in the Status window. ****/
#if 0
  if (!stop_flag){
        ret = pptp_send_ctrl_packet(conn, &rqst, sizeof(rqst));
        stop_flag++;
	log("close connect .. PPTP_CALL_CLEAR_RQST..ret[%d=%s]\n",ret, ret ?"Pass":"Fail");
  }
  else
  {
        ret = pptp_send_ctrl_packet(conn, &stop_rqst, sizeof(stop_rqst));
        stop_flag = 0;
  	log("close connect .. PPTP_STOP_CTRL_CONN_RQST..ret[%d=%s]\n",ret, ret ?"Pass":"Fail");
  }
  pptp_reset_timer();
...
int pptp_make_packet(PPTP_CONN * conn, void **buf, size_t *size) {
  ...
  while ((conn->read_size-bad_bytes) >= sizeof(struct pptp_header)) {
    ...
    /**** 20081231 modify by zhaoguang for the bug (ID:17472) 
     **** that DUT doesn't restart the PPTP control connection 
     **** when DUT receives a Set-Link-Info with an error cookie value. ****/
...
void pptp_dispatch_ctrl_packet(PPTP_CONN * conn, void * buffer, size_t size) {
  ...
  if (size < PPTP_CTRL_SIZE(ntoh16(header->ctrl_type))) {
    ...
    log("@.@ Debug Valid packet received [type: %d; length: %d].",
        (int) ntoh16(header->ctrl_type), (int) size);
  ...
  //stop_ctrl_conn_rqst :  //@.@ TODO ??
	...
	hton8(1), hton8(PPTP_GENERAL_ERROR_NONE), 0};
      ...
      if (pptp_send_ctrl_packet(conn, &reply, sizeof(reply))) {
	/**** 20081231 modify by zhaoguang for the bug (ID:17471) 
         **** that DUT doesn't send a Stop-Control-Connection-Reply to PPTP server 
         **** when server sends a Stop-Control-Connection-Request message to it. ****/
	if (conn->write_size > 0)
	{		 	
		pptp_write_some(conn); /* write as much as we can without blocking */
	}		 
   ...
   **** that DUT doesn't restart the PPTP control connection 
   **** when DUT receives a Set-Link-Info with an error cookie value. ****/
  //call_clear_rqst : //for set_link_info invalid magic_cookie
    {
  ...
  call_clear_rqst : //for set_link_info invalid magic_cookie
	...
	PPTP_CALL * call;
	vector_search(conn->call, ntoh16(packet->call_id), &call);
	pptp_call_destroy(conn, call); //@.@ original function , update for WRT54Gv8 flow-chat
  	//pptp_conn_close(conn, close_reason); //@.@ need verify 
	log("Call closed (NTFY) (call id %d)", (int) call->call_id);
      }

      /****** 2006.12.19  add for cdrouter v3.3 item 28(cdrouter_pptp_30) bug ******/
      ...
      /****** 2006.12.19 end for cdrouter v3.3 item 28(cdrouter_pptp_30) bug ******/
  ...
  case PPTP_SET_LINK_INFO:
	...
	(struct pptp_set_link_info *)buffer;
         ...
         //add by Jack 20070719 for Microsoft PPTP Server
        //@.@ for RDQA spec issue
        if (PPTP_MAGIC != ntohl(packet->header.magic))
        {
		warn("%s SET_LINK_INFO recieved invalid cookie[0x%x].\n", __FUNCTION__, ntohl(packet->header.magic));
		/**** 20081231 modify by zhaoguang for the bug (ID:17472) 
	         **** that DUT doesn't restart the PPTP control connection 
	         **** when DUT receives a Set-Link-Info with an error cookie value. ****/
		PPTP_CALL_CLEAR_RQST_FLAG=0;
                goto call_clear_rqst;
                //goto stop_ctrl_conn_rqst;  //why do it.??
                break;
        }
...
/*@.@ pptp_handle_timer(): handled echo-request & recv echo-reply Phase III*/
static void pptp_handle_timer(int sig) {
  int i;
  ...
  if (global.conn->ka_state == KA_OUTSTANDING) /*no response to keep-alive*/
...
#if 0 //@.@ Unused (have bug...)
    struct pptp_set_link_info info = {   /* for control message type 15 */
	PPTP_HEADER_CTRL(PPTP_SET_LINK_INFO ),
       ntohs(peer_id),0,0xFFFFFFFF,0xFFFFFFFF};
#endif
     ...
     * automagically? XXX
     */
    //pptp_send_ctrl_packet(global.conn, &info, sizeof(info)); //add by Jack 2007/12/12
