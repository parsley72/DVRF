...
#ifdef HNDCTF
extern u_int32_t filter_loopback_ip;

extern int is_lan_ip(u_int32_t ip);
extern bool ip_conntrack_is_ipc_allowed(struct sk_buff *skb,
		u_int32_t hooknum);
...
#ifdef HNDCTF
//Zhijian add for hndctf 2010-07-06
	...
	{
		iph = ip_hdr((*pskb));
		//print_pkt_info(ct, ctinfo, hooknum, *pskb);
		if(ct->slow_nat)
		{
			enabled = FALSE;
		}
		//Zhijian add for hndctf 2010-09-03 
		else if(iph->daddr == filter_loopback_ip && is_lan_ip(iph->saddr))
		{
			ct->slow_nat = 1;
			enabled = FALSE;
		}
		else if(iph->protocol == IPPROTO_UDP)
		{
			...
			if(is_slow_nat_port(SLOW_NAT_PROTO_UDP, uh->dest))
			{
				ct->slow_nat = 1;
				enabled = FALSE;
			}
		...
		else if(iph->protocol == IPPROTO_TCP)
		{
			...
			if(is_slow_nat_port(SLOW_NAT_PROTO_TCP, th->dest))
			{
				ct->slow_nat = 1;
				enabled = FALSE;
			}
		...
		if(enabled)
		{
			enabled = ip_conntrack_is_ipc_allowed(*pskb, hooknum);
		}
	}
	else
	{
