                ...
                struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen) {
		...
		{
			/* removed the expect tunnel here. */
			DEBUGP("found a teardown message\n");
			nf_ct_expect_conntrack_clean(ct);
			continue;
		}
		...
		rtcp_exp->master = exp->master = ct;
		rtcp_exp->expectfn = exp->expectfn = expected;
		rtcp_exp->flags = exp->flags = 0;
		...
		if (expinfo.pbtype == pb_range) {
			...
			/* setting flags for RTCP expectation. */
			rtcp_exp->flags = NF_CT_EXPECT_RTCP;
		}

		DEBUGP("rtp_expect_related %u.%u.%u.%u:%u-%u.%u.%u.%u:%u\n",
		       NIPQUAD(exp->tuple.src.u3.ip),
		       ntohs(exp->tuple.src.u.udp.port),
		       NIPQUAD(exp->tuple.dst.u3.ip),
		       ntohs(exp->tuple.dst.u.udp.port));

		DEBUGP("rtcp_expect_related %u.%u.%u.%u:%u-%u.%u.%u.%u:%u\n",
		...
		if (nf_nat_rtsp_hook)
		{
			...
			ret = nf_nat_rtsp_hook(pskb, ctinfo, hdrsoff, hdrslen, &expinfo, rtcp_exp);
		}
		else
		{
			...
			if (nf_conntrack_expect_related(rtcp_exp) != 0) {
				INFOP("nf_conntrack_rtcp_expect_related failed\n");
				ret  = NF_DROP;
			}
		}
