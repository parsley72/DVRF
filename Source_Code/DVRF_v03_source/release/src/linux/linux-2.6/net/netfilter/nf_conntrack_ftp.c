...
get_ipv6_addr(const char *src, size_t dlen, struct in6_addr *dst, u_int8_t term)
{
	...
	{
		memcpy(cStr,src,dlen);
	}
        if((pFlag=strchr(cStr,'%'))!=0)
        {
                if((pFlag_end=strchr(pFlag,'|'))!=0)
                {
			term='%';
                        int ret = in6_pton(cStr,  min_t(size_t, strlen(cStr),0xffff), (u8 *)dst, term, &end);
                        if (ret > 0)
                                return (int)((end - cStr)+(pFlag_end-pFlag));
			return 0;			
 		}
	}
        else
