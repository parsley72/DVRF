...
static int ip6mr_vif_open(struct inode *inode, struct file *file)
{
	struct seq_file *seq;
	int rc = -ENOMEM;
	struct ipmr_vif_iter *i = kzalloc(sizeof(*i), GFP_KERNEL);
...
static int ipmr_mfc_open(struct inode *inode, struct file *file)
{
	struct seq_file *seq;
	int rc = -ENOMEM;
	struct ipmr_mfc_iter *i = kzalloc(sizeof(*i), GFP_KERNEL);
...
static int ip6mr_cache_report(struct sk_buff *pkt, mifi_t mifi, int assert)
{
	...
	skb_reset_transport_header(skb);
//	msg = (struct mrt6msg *)skb_transport_header(skb);
	msg = (struct mrt6msg *)skb_network_header(skb);
...
static int ip6mr_forward2(struct sk_buff *skb, struct mfc6_cache *c, int vifi)
{
	 ...
	 */
	dev = vif->dev;
#if 0 /* if set this, outdev will equal to indev */
	skb->dev = dev;
#endif
	vif->pkt_out++;
	vif->bytes_out += skb->len;
...
int ip6_mr_input(struct sk_buff *skb)
{
	...
	if(cache == NULL)
	{
		...
		cache = ip6mr_cache_find(&in6addr_any, &ipv6_hdr(skb)->daddr);
	}
