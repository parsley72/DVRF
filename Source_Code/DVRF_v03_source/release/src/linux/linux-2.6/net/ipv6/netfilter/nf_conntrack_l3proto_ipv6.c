				      ...
				      int (*okfn)(struct sk_buff *))
{
	...
	if (ret == NF_ACCEPT)
	{
		...
		if (CTF_ENAB(kcih))
		{
			...
			{
				struct ipv6hdr *ip6h = ipv6_hdr(*pskb);
	
				if (ip6h != NULL)
				{
					u8 proto = ip6h->nexthdr;
					int offset = ipv6_skip_exthdr(*pskb, ((u_int8_t *)(ip6h+1) - (*pskb)->data), &proto);

					//printk("IPv6 packet transaltion protocol %d\n", proto);
					if(proto == IPPROTO_UDP)
					{
						struct udphdr *uh = (struct udphdr *)((unsigned char *) ip6h+offset );

						//printk("UDP packet: dest port %d\n", ntohs(uh->dest));
						if((uh != NULL) && is_slow_nat_port(SLOW_NAT_PROTO_UDP, uh->dest))
						{
							//printk("%s: UDP packet dest port %d, force to go slow nat\n", __FUNCTION__, ntohs(uh->dest));
							ct->slow_nat = 1;
							enabled = FALSE;
						}
					}
					...
					{
						struct tcphdr *th = (struct tcphdr *)((unsigned char *) ip6h+offset );

						//printk("TCP packet: dest port %d\n", ntohs(th->dest));
						if((th != NULL) && is_slow_nat_port(SLOW_NAT_PROTO_TCP, th->dest))
						{
							//printk("%s: TCP packet dest port %d, force to go slow nat\n", __FUNCTION__, ntohs(th->dest));
							ct->slow_nat = 1;
							enabled = FALSE;
						}
					}
