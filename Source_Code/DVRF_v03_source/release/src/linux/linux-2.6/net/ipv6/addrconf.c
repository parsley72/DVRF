 ...
 *
 *    	$Id: addrconf.c,v 1.2 2011/04/22 07:57:52 marcel Exp $
 *
...
struct ipv6_devconf ipv6_devconf __read_mostly = {
	...
	/* disable_ipv6 =1 for ipv6ready test addr.p2 #3 */
	.disable_ipv6           = 1,	/* hsubj copies from 2.6.30 to switch IPv6 */
	.accept_dad		= 2,	/* grace copies from 2.6.34 */
};
...
static struct inet6_dev * ipv6_add_dev(struct net_device *dev)
{
	...
	//IPv6Ready Test enable ipv6 for loopback dev by Eric	
	if (dev == &loopback_dev)
	ndev->cnf.disable_ipv6 = 0;
	//end by Eric
...
static void addrconf_disable_change(void)
{
        struct net_device *dev;
        struct inet6_dev *idev;
	...
	for_each_netdev(dev) {
                ...
                idev = __in6_dev_get(dev);
                if (idev) {
                        int changed = (!idev->cnf.disable_ipv6) ^ (!ipv6_devconf.disable_ipv6);
	      ...
	      int scope, u32 flags)
{
	...
	/* hsubj copies from 2.6.30 to switch ipv6 */
...
static void addrconf_dad_stop(struct inet6_ifaddr *ifp, int dad_failed)
{
	if (ifp->flags&IFA_F_PERMANENT) {
		...
		addrconf_del_timer(ifp);
		ifp->flags |= IFA_F_TENTATIVE;
		/* 2010-11-16 grace add linux-2.6.34 */
		if (dad_failed)
...
static int ipv6_generate_eui64(u8 *eui, struct net_device *dev)
{
	...
	/* 2010-11-16 marcel add from RV110W */
	case ARPHRD_SIT:
...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len)
{
	...
	if (pinfo->autoconf && in6_dev->cnf.autoconf) {
		...
		/*
		 * 2010-11-12 hsubj change
		 * to accept the prefix that its length isn't 64-bits in RA message
		 */
		if (/*pinfo->prefix_len == 64*/ 1) {
...
// 2011-0603 add by eric.ma (IPv6 Ready IOT)
 				run_pending = 1;
			}
		} else {
			if (!netif_carrier_ok(dev)) {
...
static int flags6_seq_show(struct seq_file *seq, void *v)
{
        struct inet6_ifaddr *ifp = (struct inet6_ifaddr *)v;
        seq_printf(seq,
                   NIP6_SEQFMT " %02x %02x %02x %02x %02x %08x %8s " NIP6_SEQFMT "\n",
                   NIP6(ifp->addr),
                   ifp->idev->dev->ifindex,
                   ifp->prefix_len,
                   ifp->scope,
		   ...
		   ifp->probes,
                   ifp->idev->if_flags,
                   ifp->idev->dev->name,
		   NIP6(ifp->idev->radvd_addr));
        return 0;
}

static struct seq_operations flags6_seq_ops = {
        .start  = if6_seq_start,
        .next   = if6_seq_next,
...
static int flags6_seq_open(struct inode *inode, struct file *file)
{
        ...
        rc = seq_open(file, &flags6_seq_ops);
        if (rc)
                goto out_kfree;
...
{
        if (!proc_net_fops_create("if_flags6", S_IRUGO, &flags6_fops))
                return -ENOMEM;
...
{
        proc_net_remove("if_flags6");
}

#if 0
/* 2010-11-17 hsubj add
   to pass the status of IPv6 address initiation to user space */
static ssize_t ifmsg6_write(struct file *file, const char __user * buf,
                            size_t count, loff_t * ppos)
{
	...
	if ((msg = strchr(data, ':')) != NULL) {
	        struct net_device *dev;
        	struct inet6_dev *idev;

		/* Cut the string to interface name and status */
		*msg = 0;
		msg++;
		...
		for (dev = dev_base; dev; dev = dev->next) {
                	rcu_read_lock();
			if (strcmp(dev->name, data) == 0) {
        		        idev = __in6_dev_get(dev);
		                if (idev) {
					strcpy(idev->stage_status, msg);
		                }
                		rcu_read_unlock();
				break;
		        }
...
static int ifmsg6_seq_show(struct seq_file *seq, void *v)
{
        struct inet6_ifaddr *ifp = (struct inet6_ifaddr *)v;
        seq_printf(seq, "%s:%s\n", ifp->idev->dev->name, ifp->idev->stage_status);
        return 0;
}

static struct seq_operations ifmsg6_seq_ops = {
        .start	= if6_seq_start,
        .next	= if6_seq_next,
        .show	= ifmsg6_seq_show,
        .stop	= if6_seq_stop,
};

static int ifmsg6_seq_open(struct inode *inode, struct file *file)
{
        struct seq_file *seq;
        int rc = -ENOMEM;
        struct if6_iter_state *s = kzalloc(sizeof(*s), GFP_KERNEL);
        ...
        rc = seq_open(file, &ifmsg6_seq_ops);
        if (rc)
                goto out_kfree;
...
static const struct file_operations ifmsg6_fops = {
        .owner	= THIS_MODULE,
        .open	= ifmsg6_seq_open,
        .read	= seq_read,
        .write	= ifmsg6_write,
        .llseek	= seq_lseek,
        .release	= seq_release_private,
...
int __init ifmsg6_proc_init(void)
{
        if (!proc_net_fops_create("if_msg6", S_IRUGO, &ifmsg6_fops))
                return -ENOMEM;

        return 0;
}

void ifmsg6_proc_exit(void)
{
        proc_net_remove("if_msg6");
}
#endif // 0
			     ...
			     u32 pid, u32 seq, int event, unsigned int flags)
{
	...
	//Jemmy remove it to fix DUT crash issue in TD test sometimes 2011.03.21
#if 0
...
/* 2011-0603 marcel add
   IR-B0017289(P2), UNH Plugfest, if the router lifetime=ZERO or EXPIRED
   from the ISP, router should update lifetime=0... */
//		if (*valp)
                           ...
                           void __user *buffer, size_t *lenp, loff_t *ppos)
{
        ...
        if (write && valp != &ipv6_devconf_dflt.disable_ipv6) {
...
static int addrconf_sysctl_disable_strategy(ctl_table *table,
                                            int __user *name, int nlen,
                                            void __user *oldval,
                                            size_t __user *oldlenp,
                                            void __user *newval, size_t newlen)
		...
		/* hsubj copies from 2.6.30 to switch ipv6 */
                {
                        ...
                        .strategy       =       &addrconf_sysctl_disable_strategy,
                },
		{
			.ctl_name	=	0,	/* sentinel */
		}
