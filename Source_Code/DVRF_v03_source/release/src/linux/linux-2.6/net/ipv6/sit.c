...
//marcel 2010.10.06  porting CONFIG_IPV6_SIT_6RD codes from linux-2.6.33
...
static int ipip6_rcv(struct sk_buff *skb)
{
	...
	if ((tunnel = ipip6_tunnel_lookup(iph->saddr, iph->daddr)) != NULL) {
		secpath_reset(skb);
		/* hsubj corrects new mac_header point. The wrong mac_header will make wrong mac_len (20) and 
		 * will make Broadcom ethernet driver crashed. */
...
// 2011-Feb-24 ruby add
// RFC-3056: IPv4 "do not fragment" bit SHOULD NOT be set in encapsulating IPv4 header.
#if 0
	if (mtu > IPV6_MIN_MTU)
...
ipip6_tunnel_ioctl (struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	//printk(KERN_EMERG "ipip6_tunnel_ioctl = %d\n", cmd);
