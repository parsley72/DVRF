...
enum ipt_trigger_status
{
	IPT_TRIGGER_STATUS_NONE, 
	IPT_TRIGGER_STATUS_DNAT
};
#endif
...
#ifdef DEL_IP_CONNTRACK_ENTRY
	enum ipt_trigger_status its;
#endif

#if defined(CONFIG_BCM_NAT) || defined(HNDCTF)
	//Zhijian add for fast nat or hndctf  2010-07-06
		unsigned int slow_nat;
#endif

#ifdef CONFIG_DNSHOOK_MODULE
	//tlhhh add for traffic filter whitelist.
...
/* Lai 2010.04.02 add to remove the expectation conntrack. */
