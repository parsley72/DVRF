...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
		...
		*skb_push(skb, 2) = 1;
		...
		else
		{
			if(IP_PROTO(skb) != 1) //don't ignore ping packet!!
			{
				...
				{
					//myprintk("self %s send dns ppp frame, skip!!\n", current->comm);
					skip = 1;
				}
			}
	 ...
	 */
	//if (ppp->flags & SC_LOOP_TRAFFIC) { //Lai 2010.08.02 add for COD
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (npi < 0) {
		...
		*skb_push(skb, 2) = 0;
		...
		skb_pull(skb, 2);
#else
		;//ppp->last_recv = jiffies;/*Taide modify for pptp demond 2010_09_20*/
#endif /* CONFIG_PPP_FILTER */
