...
unsigned int force_usb_mode=0; /* 2011-07-12, jemmy add to resolve CBTS#27697(P2) */
...
free_interfaces:
	...
	//Jemmy add for multi-function USB device		
	printk("%s: has interfaces %d\n", __FUNCTION__, nintf);
	force_usb_mode = 0;
	if (nintf > 1)
	{
		...
		for (i = 0; i < nintf; ++i) {
			...
			//printk("Jemmy: interface %d class %x\n", i, intf->cur_altsetting->desc.bInterfaceClass);
			if (intf->cur_altsetting->desc.bInterfaceClass == USB_CLASS_MASS_STORAGE)
				storage_dev = 1;
			else if (intf->cur_altsetting->desc.bInterfaceClass == USB_CLASS_HID)
				hid_dev = 1;
			else
				others_dev = 1;
		}

		/* if once have other device, force usb mode to virtualUSB mode
 		*  if only have HID device , also force usb mode to VirtualUSB mode
 		*  if have both HID and Storage, then force usb mode to storage mode
 		*  if only have storage device, also force usb mode to storage mode
 		*/
		if (others_dev == 1)
			force_usb_mode = 1;
		else if (hid_dev == 1)
