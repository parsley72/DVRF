...
unsigned int usb_mode = 3;
extern unsigned int force_usb_mode; /* 2011-07-12, jemmy add to resolve CBTS#27697(P2) */
static struct ctl_table_header *usbmode_sysctl_header;
	...
	struct usb_endpoint_descriptor *endpoint)
{
	...
	//Jemmy delete it for support HUB in virtual USB mode 2011.2.10
	//add by junzhao 2010.9.25 for blocking USB hub
#if 0
	printk("dev %s parent!\n", hdev->parent?"Has":"Has not");
	if(hdev->parent)
	{
...
void usb_disconnect(struct usb_device **pdev)
{
	...
	usb_mode = 3;
	force_usb_mode = 0; /* 2011-07-12, jemmy add to resolve CBTS#27697(P2) */
					...
					u16 portstatus, u16 portchange)
{
	...
	for (i = 0; i < SET_CONFIG_TRIES; i++) {
		...
		dev_dbg(hub_dev, "usb new deivce %d\n", port1);
...
static void *led_flag_start(struct seq_file *s, loff_t *ppos)
{
	if (*ppos == 0)
		return (void *)1;
	else
...
static void *led_flag_next(struct seq_file *s, void *v, loff_t *pos)
{
	++*pos;
	return NULL;
}
...
static void led_flag_stop(struct seq_file *s, void *v)
{
}
...
static int led_flag_show(struct seq_file *s, void *v)
{
	if (v == (void *)1) {
		if (list_empty(&hub_event_list) && usb_led_flag)
			seq_puts(s, "1");
		else
			seq_puts(s, "0");
	}
...
static struct seq_operations led_flag_seq_ops = {
	.start		= led_flag_start,
	.next		= led_flag_next,
	.stop		= led_flag_stop,
	.show		= led_flag_show,
};
...
static int usb_led_flag_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &led_flag_seq_ops);
}
...
static int usb_led_probe_task(void *x)
{
	...
	do {
		...
		interruptible_sleep_on_timeout(&usb_led_blink_queue, 10);
		if (signal_pending(current))
			break;
...
static struct file_operations proc_usb_led_flag_operations = {
	open:		usb_led_flag_open,
	read:		seq_read,
	llseek:		seq_lseek,
	release:	seq_release,
};
...
int usb_hub_init(void)
{
	...
	struct proc_dir_entry *proc_usb_led_flag;
#endif
	pid_t pid;
	...
	if (usb_register(&hub_driver) < 0) {
        ...
        if (proc_usb_scan_done == NULL) {
                printk(KERN_ERR "cannot init /proc/usb/scan_done\n");
                return -ENOMEM;
        }
        proc_usb_scan_done->proc_fops = &proc_usb_scan_done_operations;
                                                                                                               
        proc_usb_led_flag = create_proc_entry("led_flag", 0, proc_usb);
        if (proc_usb_led_flag == NULL) {
                printk(KERN_ERR "cannot init /proc/usb/led_flag");
                return -ENOMEM;
        }
        proc_usb_led_flag->proc_fops = &proc_usb_led_flag_operations;

#endif
	usbmode_sysctl_header = register_sysctl_table(mytable);                                                                               
...
void usb_hub_cleanup(void)
{
	 ...
	 */
	usb_deregister(&hub_driver);
	unregister_sysctl_table(usbmode_sysctl_header);
} /* usb_hub_cleanup() */
...
EXPORT_SYMBOL(usb_led_flag);
EXPORT_SYMBOL(usb_led_queue);
