 ...
 *
 * $Id: bcmrobo.c,v 1.3 2010/12/21 01:43:23 gavin.ke Exp $
 */
...
#define REG_MII_CLAUSE_45_CTL1	0x0d	/* 53125 MII Clause 45 control 1 */
#define REG_MII_CLAUSE_45_CTL2	0x0e	/* 53125 MII Clause 45 control 2 */
...
#define REG_CTRL_53125_LED	0x12	/* 53125: LED */

#define REG_VALUE_53125_LED	0x20B	/* 53125: LED command: et robowr 0x0 0x12 0x20b */
...
bcm_robo_enable_switch(robo_info_t *robo)
{
	...
	if (/*(robo->devid == DEVID53115) || */(robo->devid == DEVID53125)) {
		...
		robo_eee_advertise_crtl(robo, 0, -1); /* disbale eee advertise in all phys */

		/* 
		 * Write LED function 1 control register(Page 00h: Addr.12h) to 0x020B and 
		 * LED0 will be SPD1G, LED1 will be ACT, LED2 will be LNK and LED3 will be PHYLED4
		 */
		val16 = REG_VALUE_53125_LED;
		robo->ops->write_reg(robo, PAGE_CTRL, REG_CTRL_53125_LED, &val16, sizeof(val16));
	}
...
/*
 * Update EEE advertise register in phy.
 * enable: 0:disable eee  1:enable eee
 * phy: -1:all phys x:specific phy starting from 0
 */
void
robo_eee_advertise_crtl(robo_info_t *robo, int32 enable, int32 phy)
{
	uint16 val16;
	int32 start, end;

	ASSERT(robo);
	ASSERT((phy >= -1) && (phy < MAX_NO_PHYS));
	...
	for (phy = start; phy < end; phy++) {
		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL1, 0x0007); /* select DVEAD 7 */
		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL2, 0x003c); /* select Register 3Ch */
		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL1, 0xc007); /* read command */
		val16 = robo->miird(robo->h, phy, REG_MII_CLAUSE_45_CTL2); /* read data */
		 ...
		 */
		if (enable) {
			val16 |= 0x06; //set bit 1 and 2 */
		} else {
			val16 &= (~0x06); // clear bit 1 and 2 */
		}

		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL1, 0x0007); /* select DVEAD 7 */
		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL2, 0x003c); /* select Register 3Ch */
		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL1, 0x4007); /* write command */
		robo->miiwr(robo->h, phy, REG_MII_CLAUSE_45_CTL2, val16); /* write data */
	}
