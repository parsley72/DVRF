...
static void lease_mac_timeout(void *vlp);
struct iaddr *find_ip_from_dhcp_statics(struct hardware hw_addr, struct iaddr *lease_ip);
static int hw_addr_eq(struct hardware addr1, struct hardware addr2);
...
#ifdef HAVE_DHCPD_LOGFILE
{
	...
	if (offer == DHCPOFFER && !(lease -> flags & STATIC_LEASE) &&
		...
		if (!(lease->flags & STATIC_LEASE) &&
			...
			dhcp_reply(lease);
		/*
		* delay some time (1s, or 2s), then check whether the client mac
		* and the mac mapping to the lease ip int arp cache
		* are same.
		* John
		*/
		struct iaddr lease_ip;
		if (find_ip_from_dhcp_statics(lease->hardware_addr, &lease_ip)){
			if (addr_eq(lease->ip_addr, lease_ip)) {
				/*First delete the static entry in arp table*/
 				eval("/usr/sbin/arp","-d",piaddr(lease->ip_addr));
			}
		}

		tv . tv_sec = cur_time + 2;
		tv . tv_usec = 0;
		add_timeout (&tv, lease_mac_timeout, lease, NULL, NULL);
	}
}
...
/*
*Check mac int arp table mapping lease ip
*and lease client mac
*John
*/
static void
lease_mac_timeout(void *vlp)
{
	struct lease *lease =  vlp;
	char *arp_mac = NULL;
	char *lease_mac = NULL;
	
	arp_mac = get_mac_from_ip(piaddr(lease -> ip_addr)) ;
	
	if (!arp_mac || !*arp_mac)
		return;
	lease_mac = print_hw_addr (lease -> hardware_addr.hbuf [0],
				    lease -> hardware_addr.hlen - 1,
				    &lease -> hardware_addr.hbuf [1]) ;
	if (strcasecmp(arp_mac, lease_mac)) {
		lease -> flags |= RESERVED_LEASE_WEIRD ;
	} else { /*add static arp entry for this lease*/
		struct iaddr lease_ip;
		if (find_ip_from_dhcp_statics(lease->hardware_addr, &lease_ip)){
			if (addr_eq(lease->ip_addr, lease_ip)) {
				eval("/usr/sbin/arp","-s",
					piaddr (lease -> ip_addr), lease_mac);
			}
		}
 	}

	cancel_timeout(lease_mac_timeout, lease);
	/* 2011-07-19, sync E4200 Wake-on-LAN support */
	//lease_dereference (&lease, MDL);
	return;
}
...
/*John@2011.02.20 fetch ip from dhcp_statics nvram buffer*/
struct iaddr *
find_ip_from_dhcp_statics(struct hardware hw_addr, struct iaddr *lease_ip)
{
	char *dhcp_statics = nvram_safe_get("dhcp_statics");
	char *lan_ipaddr  = nvram_safe_get("lan_ipaddr");
	char *lan_netmask = nvram_safe_get("lan_netmask");
	struct in_addr ipaddr, netmask, subnet, find_ip_addr;
	...
	_foreach(word, dhcp_statics, next, ";", ';') {
		struct hardware mac_addr;
		unsigned char eth_addr[7];

		sscanf(word, "%s %s %s %s", mac, ip, enable, name);
		if (!strcmp(enable, "on")) {
			memset(eth_addr, 0, sizeof(eth_addr));
			eth_addr[0] = 1; //hardware type, ethernet=1
			ether_atoe(mac, eth_addr + 1);
			mac_addr.hlen = ETHER_ADDR_LEN + 1;
			memcpy(mac_addr.hbuf, &eth_addr, mac_addr.hlen);

			if (hw_addr_eq(mac_addr, hw_addr)){
				find_ip_addr.s_addr = subnet.s_addr | ((atoi(ip) << 24) & ~netmask.s_addr);
				if (lease_ip) {
					lease_ip->len = sizeof(struct in_addr);
					memcpy(lease_ip->iabuf, &find_ip_addr, sizeof(struct in_addr));
					return lease_ip;
				}
			}
		}
...
static int hw_addr_eq(struct hardware addr1, struct hardware addr2)
{
	if (addr1.hlen > sizeof(addr1.hbuf))
		log_fatal("hw_addr_eq():%s:%d: Invalid hw addr length.", MDL);

	if (addr1.hlen != addr2.hlen)
		return 0;
	return memcmp (addr1.hbuf, addr2.hbuf, addr1.hlen) == 0;
}
