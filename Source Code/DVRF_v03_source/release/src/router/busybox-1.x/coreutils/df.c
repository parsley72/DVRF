...
#include <sys/vfs.h>
#include "libbb.h"
/*John cast octa character to normal char*/
...
int df_main(int argc, char **argv)
{
 ...
 SET_ERROR:
		...
		mount_point_cast = strdup(mount_entry->mnt_dir);
		
		/*John*/
		octal_char_cast(mount_point_cast, mount_point);
		free(mount_point_cast);
...
/*
*convert octal constant char to ASCII char,
*for example, cast \040 to a space char
*John 2011.03.11
*/
char * octal_char_cast(const char *src, char *dest)
{
	...
	while (*sch != 0 ) {
		if ((*sch == '\\')
				...
				&& (*(sch+2) >= '0' &&  *(sch+2) < '8')
				&& (*(sch+3) >= '0' &&  *(sch+3) < '8')){
			*dch++ = (*(sch+1) - '0') * 64 + (*(sch+2) - '0') * 8 + (*(sch+3) - '0') ;
			sch += 4;
		} else 
			*dch++ = *sch++;
	}
