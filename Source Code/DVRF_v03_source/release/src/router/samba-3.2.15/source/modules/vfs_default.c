...
#include "includes.h"
/*John@2010.12.09 add setmntent getmntent endmntent support*/
#include <mntent.h>
#define HAVE_MNTENT 1
#define MOUNTTABLE "/proc/mounts"
...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, SMB_OFF_T len)
{
	...
	/*John@2010.12.09,as of the version of uClibc-0.9.29 , it spports ftruncate64 not well
	 *for FAT32 filesytem,if the file size is more than 350M or so,it will cost so much time 
	 *that client often donot get reponse to the request of TRANS2 SET_FILE_INFO in time, 
	 *so uploading file to samba server failed, however, it is lunky that we can skip over 
	 *this step  by the lseek,fstat call as follows, however, if ntfs mounted, ftruncated work
	 *work, so if ntfs, we call it, skip otherwise*/
	if (SMB_VFS_FSTAT(fsp, &st1) == -1) {
        	goto done;
	}
	devno = st1.st_dev;
        
	if ((fp = setmntent(MOUNTTABLE, "r")) == NULL ) 
		goto done ;

	while ((mnt = getmntent(fp)) != NULL){
		if ( sys_stat(mnt->mnt_dir,&st1) == -1 ){
			continue;
		}
		if ( (major(st1.st_dev) == major(devno))
			 && (minor(st1.st_dev) == minor(devno))
             && ( !strcmp(mnt->mnt_type, "ufsd") || !strcmp(mnt->mnt_type, "ntfs")) ){
				is_ntfs = 1;
				break;
		}
	}
