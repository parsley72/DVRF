...
//end by michael
  ...
  {
    //return 0;
	if( header->opcode > 0x04)
	    return -1;	//add by michael to fix bt crash dnsmasq at 20090309
	else 
	    return 0;
  }
  ...
  if (!(ansp = skip_questions(header, qlen)))
    //return 0; /* bad packet */
   	return -1; //add the patch from ethan at 20090309
...
//add by michael to add the router_url_address at 2008-3-5
{
	...
	strncpy(ipaddr,lan_ipaddr,sizeof(ipaddr));
	strncpy(url_tmp,router_url_address,sizeof(url_tmp));
#endif
 	if(!strstr(url_tmp,"://"))
		strcpy(url,url_tmp);
	else
	...
	//fprintf(stderr,"the ipaddr is:[%s] and url_emp is:[%s],and url is:[%s]!\n",ipaddr,url_tmp,url);
