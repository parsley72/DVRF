...
__nf_conntrack_expect_find_by_sip_call_id(unsigned int sip_call_id_hash)
{
	struct nf_conntrack_expect *i;

	list_for_each_entry(i, &nf_conntrack_expect_list, list) {
		if ((i->sip_call_id_hash != 0) && (sip_call_id_hash == i->sip_call_id_hash)) {
				return i;
		}
	}
	return NULL;
}
EXPORT_SYMBOL_GPL(__nf_conntrack_expect_find_by_sip_call_id);

struct nf_conntrack_expect *
nf_conntrack_expect_find_by_sip_call_id(unsigned int sip_call_id_hash)
{
	struct nf_conntrack_expect *i;

	read_lock_bh(&nf_conntrack_lock);
	i = __nf_conntrack_expect_find_by_sip_call_id(sip_call_id_hash);
	if (i)
		atomic_inc(&i->use);
...
EXPORT_SYMBOL_GPL(nf_conntrack_expect_find_by_sip_call_id);
