...
static int mangle_http_header(const struct sk_buff *skb, int flags)
{
    ...
    tcph = skb_header_pointer(skb, start+sizeof(_ip6h), sizeof(_tcph), &_tcph);
    ...
    if (memcmp(data, "GET ", sizeof("GET ") - 1) != 0 &&
        ...
        memcmp(data, "HEAD ", sizeof("HEAD ") - 1) != 0) //Fixed 2010/08/03
	return ret;	/* Pass it */
...
static int get_http_info(const struct sk_buff *skb, int flags, httpinfo_t *info)
{
    ...
    if (memcmp(data, "GET ", sizeof("GET ") - 1) != 0 &&
        memcmp(data, "POST ", sizeof("POST ") - 1) != 0 &&
	 memcmp (data, "HEAD ", sizeof ("HEAD ") - 1) != 0) //Fixed
	return ret;	/* Pass it */
    ...
    *(info->host + hostlen) = 0;		/* null-terminated */
    ...
    if (!found) //Fixed 2010/08/03
        found = find_pattern2 (data, datalen, "HEAD ",  //Fixed
...
//        if (!strcmp (needle, "XXX.XXX.XXX.XXX"))//Fixed 2010/08/03
  //              return t;//
...
//      u_int16_t datalen,
      bool *hotdrop)
{
	const struct ip6t_webstr_info *info = matchinfo;
	struct ipv6hdr _ip6h, *ip6h;
	proc_ip6t_search search=search_linear;
	...
	switch(info->type)
	{
	    case IP6T_WEBSTR_URL:	/* fall through */
		flags |= HTTP_URL;
	...
	if (info->type == IP6T_WEBSTR_HOST || info->type == IP6T_WEBSTR_URL) {
		...
		SPARQ_LOG("keyword=%s, nlen=%d, hlen=%d\n", needle, nlen, hlen);
            	...
            	if (info->type == IP6T_WEBSTR_HOST) //Fixed
                {
                	if (!nlen || !hlen || nlen != hlen)
                        	continue;
                }
                else if (!nlen || !hlen || nlen > hlen) //Fixed
			continue;
